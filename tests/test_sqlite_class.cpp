...
{
    BOOST_CHECK_NO_THROW(cpp_db::statement("create table TEST_TABLE(COL1 INT NOT NULL, COL2 VARCHAR(50));", *con.get()).execute_non_query());
    BOOST_CHECK_NO_THROW(cpp_db::statement("insert into TEST_TABLE(COL1, COL2) VALUES(1, 'first')", *con.get()).execute_non_query());
    BOOST_CHECK_NO_THROW(cpp_db::statement("insert into TEST_TABLE(COL1, COL2) VALUES(2, 'second')", *con.get()).execute_non_query());
    BOOST_CHECK_NO_THROW(cpp_db::statement("insert into TEST_TABLE(COL1, COL2) VALUES(3, 'third')", *con.get()).execute_non_query());
	BOOST_CHECK_THROW(cpp_db::statement("select COL1, COL2, COL3 from TEST_TABLE;", *con.get()).execute_non_query(), cpp_db::db_exception);
}
...
BOOST_AUTO_TEST_CASE(test_result)
{
    ...
    BOOST_CHECK_THROW(cpp_db::result(cpp_db::statement(*con.get()).execute()), cpp_db::db_exception);
}
BOOST_AUTO_TEST_CASE(test_parameter)
{
    ...
    BOOST_CHECK_EQUAL(cpp_db::value_of<int64_t>(cpp_db::statement("select count(*) from TEST_TABLE where (COL1 = 4 and COL2 = 'four') OR (COL1 = 5 and COL2 = 'five')", *con.get()).execute_scalar()), 2);
    ...
    BOOST_CHECK_EQUAL(cpp_db::value_of<int64_t>(cpp_db::statement("select count(*) from TEST_TABLE where COL1=99 and COL2='Unknown'", *con.get()).execute_scalar()), 3);

    BOOST_CHECK_EQUAL(cpp_db::value_of<int64_t>(cpp_db::statement("select count(*) from TEST_TABLE where COL1=? and COL2=?", *con.get()).execute_scalar(99, "Unknown")), 3);
}
...
BOOST_AUTO_TEST_CASE(test_transaction_commit)
{
    ...
    BOOST_CHECK_EQUAL(cpp_db::value_of<int64_t>(cpp_db::statement("select count(*) from TEST_TABLE", *con.get()).execute_scalar()), 8);

    BOOST_CHECK_NO_THROW(tran.begin());
    cpp_db::statement stmt(*con.get());
    BOOST_CHECK_NO_THROW(stmt.prepare("delete from TEST_TABLE where COL1 = ?"));
    BOOST_CHECK_NO_THROW(stmt.execute_non_query(99));
    BOOST_CHECK_EQUAL(cpp_db::value_of<int64_t>(cpp_db::statement("select count(*) from TEST_TABLE", *con.get()).execute_scalar()), 5);
...
BOOST_AUTO_TEST_CASE(test_transaction_rollback)
{
    ...
    cpp_db::statement stmt(*con.get());
    BOOST_CHECK_NO_THROW(stmt.prepare("delete from TEST_TABLE where COL1 > ?"));
    BOOST_CHECK_NO_THROW(stmt.execute_non_query(3));
    BOOST_CHECK_EQUAL(cpp_db::value_of<int64_t>(cpp_db::statement("select count(*) from TEST_TABLE", *con.get()).execute_scalar()), 3);
...
{
    BOOST_CHECK_EQUAL(cpp_db::value_of<int64_t>(cpp_db::execute_scalar(*con.get(), "select count(*) from TEST_TABLE")), 5);
    ...
    BOOST_CHECK_EQUAL(cpp_db::value_of<int64_t>(cpp_db::execute_scalar(*con.get(), "select count(*) from TEST_TABLE where COL1 in (?, ?) and COL2 in (?, ?)", 1, 2, "first", "second")), 2);
    BOOST_CHECK_NO_THROW(cpp_db::execute_non_query(*con.get(), "delete from TEST_TABLE where COL1 = ?", 1));
    BOOST_CHECK_EQUAL(cpp_db::value_of<int64_t>(cpp_db::execute_scalar(*con.get(), "select count(*) from TEST_TABLE")), 4);
    BOOST_CHECK_NO_THROW(cpp_db::execute_non_query(*con.get(), "delete from TEST_TABLE"));
    BOOST_CHECK_EQUAL(cpp_db::value_of<int64_t>(cpp_db::execute_scalar(*con.get(), "select count(*) from TEST_TABLE")), 0);
}
