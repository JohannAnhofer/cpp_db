...
void junit_output::start_class(const std::string &class_name)
{
	...
	test_suites.insert(std::make_pair(class_name, suite));
}
...
void junit_output::start_function(const std::string &class_name, const std::string &function_name)
{
	...
	test_suites[class_name].test_functions.insert(make_pair(function_name, function));
	current_test_function = &test_suites[class_name].test_functions[function_name];
}

void junit_output::end_function(const std::string &class_name, const std::string &function_name)
{
	test_suites[class_name].test_functions[function_name].end = std::chrono::system_clock::now();
	current_test_function = nullptr;
}

void junit_output::end_class(const std::string &class_name, const test_class_statistics &stats)
{
	auto &test_suite = test_suites[class_name];
...
void junit_output::add_test_case(const std::string &type, const std::string &message, int line, const char * file)
{
...
{
	add_test_case(std::string(), message, line, file);
	(void)line;
    (void)file;
}
...
{
	add_test_case("failure", message, line, file);
    (void)line;
    (void)file;
}
...
{
	add_test_case("error", message, line, file);
    (void)line;
    (void)file;
}
...
inline double duration_in_seconds(const timepoint &start, const timepoint &end)
{
	return std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() / 1000.0;
}

void junit_output::write_xml() const
{
